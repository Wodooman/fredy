...
import { recordJobRun } from '../storage/healthStorage.js';
...
export function initJobExecutionService({ providers, settings, intervalMs }) {
  ...
  /**
   * Resolve admin user IDs for health update SSE.
   * @returns {string[]}
   */
  function resolveAdminRecipients() {
    const admins = (userStorage.getUsers && userStorage.getUsers(false)) || [];
    return admins.filter((u) => u.isAdmin).map((u) => u.id);
  }
   ...
   * Records the run and per-provider results to health tracking storage.
   *
   * @param {Object} job
   * @param {string} job.id
    ...
    try {
      ...
      for (const prov of jobProviders) {
        ...
        providerResults.push({
          ...
          errorMessage,
          listingsFound: tracer.getListingsFound(),
          newListings: tracer.getNewListings(),
          executionLog: tracer.getLog(),
        });
      }
    } finally {
      if (browser) {
      ...
      const hasFailure = providerResults.some((p) => p.status === 'failure');
      try {
        const runId = recordJobRun({
          jobId: job.id,
        ...
        // Emit health update to admin users via SSE
        const adminIds = resolveAdminRecipients();
        if (adminIds.length > 0) {
          sendToUsers(adminIds, 'healthUpdate', {
            jobId: job.id,
            healthStatus: hasFailure ? 'red' : 'green',
            runId,
          });
        }
      } catch (err) {
        logger.warn('Failed to record health tracking data for job', job.id, err);
      }
